...
def _photo_hash(image_path, box: list[int] | None) -> str | None:
    """64-bit average hash of the portrait region, as a hex string.
...
def save_scan(result: dict[str, Any], *, image_path=None, db_path: str | Path = DEFAULT_DB
             ) -> None:
        ...
        conn.execute(
            """INSERT OR REPLACE INTO scans
               (scan_id, created_at, doc_type, name, document_no, dob, photo_hash,
                risk_score, risk_level, final_status, extracted_json, validation_json,
...
def find_multiple_identity(photo_hash: str | None, *, name: str | None, document_no: str | None,
    ...
    for r in rows:
        ...
        if same_name and same_doc:
            continue                                     # the same document, not a conflict
