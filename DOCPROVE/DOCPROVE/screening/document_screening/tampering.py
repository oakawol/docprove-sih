...
def analyse(img: Image.Image, ocr_result, cross_signals: dict[str, list[str]] | None = None,
    ...
    heat = build_heatmap(maps, regions, scores)
    diag = {
...
def build_heatmap(maps: dict[str, np.ndarray], regions: dict[str, list[int]],
    ...
    for name, box in regions.items():
        ...
        x1, y1 = min(shape[1], x1 + 6), min(shape[0], y1 + 6)
        gate[y0:y1, x0:x1] = np.maximum(gate[y0:y1, x0:x1],
                                        np.clip((s - 1.0) / (HIGH_THRESHOLD - 1.0), 0, 1))
    gate = cv2.GaussianBlur(gate, (0, 0), 12)
    heat = np.clip(base * (0.12 + 1.6 * gate), 0, 1)
    if heat.max() > 0:
        heat /= heat.max()
    return heat
...
def render_heatmap(img: Image.Image, heat: np.ndarray, findings: list[Finding],
                   *, alpha: float = 0.62) -> Image.Image:
    """Thermal overlay: cool where the page is clean, red where it is suspicious."""
    rgb = np.asarray(img.convert("RGB")).astype(np.float32)
    gray = cv2.cvtColor(rgb.astype(np.uint8), cv2.COLOR_RGB2GRAY)
    base = cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB).astype(np.float32)
    base = base * 0.55 + 60                               # dim the page so heat pops

    colour = cv2.applyColorMap((np.clip(heat, 0, 1) * 255).astype(np.uint8),
                               cv2.COLORMAP_TURBO)
    colour = cv2.cvtColor(colour, cv2.COLOR_BGR2RGB).astype(np.float32)

    a = (np.clip(heat, 0, 1) ** 0.75)[:, :, None] * alpha
    ...
    for f in findings:
        if f.field == "copy_move":
            continue
        x0, y0, x1, y1 = f.box
        col = (255, 40, 40) if f.severity == "high" else (255, 150, 30)
        d.rectangle([x0 - 5, y0 - 5, x1 + 5, y1 + 5], outline=col, width=3)
        tag = f"{f.label}  {f.score:.1f}"
        fnt = _font(16)
        tw = d.textlength(tag, font=fnt)
        ty = max(0, y0 - 27)
        d.rectangle([x0 - 5, ty, x0 + tw + 12, ty + 22], fill=col + (235,))
        d.text((x0 + 2, ty + 3), tag, font=fnt, fill=(255, 255, 255))
    return out_img


def render_annotated(img: Image.Image, findings: list[Finding]) -> Image.Image:
    """The original document with the suspect fields ringed and shaded red."""
    ...
    for f in findings:
        ...
        d.rectangle([x0 - 6, y0 - 6, x1 + 6, y1 + 6], fill=col + (58,), outline=col + (255,),
                    width=3)
        tag = f.label if f.field != "copy_move" else "Copy-paste block"
        fnt = _font(17)
        tw = d.textlength(tag, font=fnt)
        ty = y1 + 8 if y0 < 40 else y0 - 30
        d.rectangle([x0 - 6, ty, x0 + tw + 14, ty + 25], fill=col + (240,))
        d.text((x0 + 2, ty + 4), tag, font=fnt, fill=(255, 255, 255))
    return out
